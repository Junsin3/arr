...
\citep{liu2025multiui}. 총 148K개의 유효한 사실 레코드를 얻었으며 두 설명
...
110.2단어, 4.24문장이다.
표 \ref{tab:data}의 전체 구축 규모에는 각 조건의 유효 샘플 수를
그대로 사용한다. 상세도의 통제 비교에는 Short와 Long이 모두
유효한 동일 screenshot의 교집합만 사용하며, 이 subset에서는
학습 sample 수와 update 수를 맞춘다.

\input{tables/data_composition}

\paragraph{프로그램 기반 검증.}
Short에는 좌표ㆍ길이 제약 검증을 적용하고, Long에는 반복 퇴화
필터를 적용한다. 모든 Short 생성 결과의 좌표를 Stage 1 좌표
집합과 대조한다.
